...
if submit_button:
    ...
    try:
        ...
        if prediction >= risk_cutoff:
            ...
            # Provide recommendations for each feature
            if lym < 0.8:
                st.markdown(f"<span style='color:red'>Lym (10^9/L): Your value is {lym}. It is lower than the normal range (0.8 - 4.0). Consider increasing it towards 0.8.</span>", unsafe_allow_html=True)
            elif lym > 4.0:
                st.markdown(f"<span style='color:red'>Lym (10^9/L): Your value is {lym}. It is higher than the normal range (0.8 - 4.0). Consider decreasing it towards 4.0.</span>", unsafe_allow_html=True)
            else:
                st.write(f"Lym (10^9/L): Your value is within the normal range (0.8 - 4.0).")

            if hb < 120:
                st.markdown(f"<span style='color:red'>Hb (g/L): Your value is {hb}. It is lower than the normal range (120 - 170). Consider increasing it towards 120.</span>", unsafe_allow_html=True)
            elif hb > 170:
                st.markdown(f"<span style='color:red'>Hb (g/L): Your value is {hb}. It is higher than the normal range (120 - 170). Consider decreasing it towards 170.</span>", unsafe_allow_html=True)
            else:
                st.write(f"Hb (g/L): Your value is within the normal range (120 - 170).")

            if alb < 35:
                st.markdown(f"<span style='color:red'>Alb (g/L): Your value is {alb}. It is lower than the normal range (35 - 50). Consider increasing it towards 35.</span>", unsafe_allow_html=True)
            elif alb > 50:
                st.markdown(f"<span style='color:red'>Alb (g/L): Your value is {alb}. It is higher than the normal range (35 - 50). Consider decreasing it towards 50.</span>", unsafe_allow_html=True)
            else:
                st.write(f"Alb (g/L): Your value is within the normal range (35 - 50).")

            # Drug therapy recommendation based on user selection
            if beta_blocker == 0:
